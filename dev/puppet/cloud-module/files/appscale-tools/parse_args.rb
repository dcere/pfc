...
EC2_LICENSE_WARNING = "Warning: The EC2 tools license forbids running the tools" +
  " against any infrastucture other than Amazon resources."
...
### Modifications
if arg_hash['cp_user']
   CP_USER = arg_hash['cp_user']
else
   CP_USER = nil
end

if arg_hash['cp_pass']
   CP_PASS = arg_hash['cp_pass']
else
   CP_PASS = nil
end
...
if arg_hash['infrastructure'] 
  ...
  abort(INFRASTRUCTURE_FLAG_NOT_IN_SET_MSG) unless VALID_CLOUD_TYPES.include?(infra)

  if infra == "ec2"
    puts EC2_LICENSE_WARNING
  end
...
if arg_hash['ips']
  ...
  if arg_hash['min'] or arg_hash['max']
    no_min_max = "When using the ips flag, the min or max flags cannot be " +
      "specified. Please only use either (1) the min and max flags or " + 
      "(2) the ips flag."
    abort(no_min_max)
  end
