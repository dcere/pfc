...
def start_vm(vm, ip_roles, img_roles, pm_up)
   ...
   domain_file = File.open("/etc/puppet/modules/web/files/#{domain_file_name}", 'w')
   ...
   command = "scp /etc/puppet/modules/web/files/#{domain_file_name}" +
...
#    machines without changing anything, so not really.
   ...
   ips.each do |vm|
      ...
      if vm != MY_IP
         ...
         file = "init-web.pp"
         path = "/etc/puppet/modules/web/manifests/#{file}"
...
def auto_manage()
   ...
   path = "/etc/puppet/modules/web/files/cron/#{cron_file}"
