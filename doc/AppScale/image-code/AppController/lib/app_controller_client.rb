...
class AppControllerClient
  ...
  def get_status(ok_to_fail=false)
    make_call(10, !ok_to_fail, "get_status", ok_to_fail) { @conn.status(@secret) }
  end
  ...
  def done()
    make_call(30, RETRY_ON_FAIL, "done", ok_to_fail=true) { @conn.done(@secret) }
  end
  ...
  def backup_appscale(backup_info)
    make_call(NO_TIMEOUT, RETRY_ON_FAIL, "backup_appscale") { @conn.backup_appscale(backup_info, @secret) }
  end
